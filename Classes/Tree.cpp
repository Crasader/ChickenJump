...
void Tree::spawn(cocos2d::Layer* layer) {
    ...
    _tree = Sprite::create(String::createWithFormat("tree%i.png", treeType)->getCString());
//    _tree->setAnchorPoint(Vec2(0, 0));
    
    int randDelayWidth = CCRANDOM_0_1() * 4 + 1;
    _tree->setPosition(Vec2(_origin.x + _visibleSize.width + _tree->getContentSize().width * randDelayWidth, _origin.y + (_visibleSize.height / 7) * 2 ));
...
//    auto action = MoveTo::create(TREE_SPEED * _visibleSize.width, Point(-_tree->getContentSize().width * (6 - random), _tree->getPositionY()));
    _tree->runAction(action);
    
    layer->addChild(_tree);
}