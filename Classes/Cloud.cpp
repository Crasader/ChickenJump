...
void Cloud::spawn(cocos2d::Layer* layer) {
    ...
    _sprite = Sprite::create(String::createWithFormat("cloud%i.png", cloudType)->getCString());
    _sprite->setAnchorPoint(Vec2(0, 0));
    
    int maxDelayWidth = 3;
    int randDelayWidth = CCRANDOM_0_1() * maxDelayWidth + 1; // range=1~4
    int posX = _visibleSize.width + _sprite->getContentSize().width * randDelayWidth;

    int heightRange = _visibleSize.height / 4 - _sprite->getContentSize().height;
...
//    auto action = MoveBy::create(CLOUD_SPEED * _visibleSize.width, Point(- _visibleSize.width * 1.5, 0));
    ...
    auto action = MoveTo::create(CLOUD_SPEED * _visibleSize.width,
                                 Point(-_sprite->getContentSize().width * ((maxDelayWidth + 1) - randDelayWidth), _sprite->getPositionY()));
    _sprite->runAction(action);
    
    layer->addChild(_sprite, BackgroundLayer::layerBackground);
}